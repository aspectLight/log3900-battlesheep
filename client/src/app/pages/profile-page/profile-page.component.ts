...
export class ProfilePageComponent implements OnInit {
    ...
    form = inject(FormBuilder).nonNullable.group({
        username: ['', [Validators.required, Validators.pattern(/^[a-zA-Z0-9]+$/), bannedUsernameValidator]],
    ...
    private customChannelService = inject(CustomChannelService);
    private avatarRegistry = inject(AvatarRegistryService);
    ...
    onAvatarFileSelected(event: Event) {
        ...
        // Reset l'attribut value pour que re-sélectionner le même fichier re-déclenche le change
        ...
        try {
            ...
            if (hasAvatarFile && this.selectedAvatarFile) {
                ...
                if (updatedProfileWithAvatar.avatarUrl) {
                    this.avatarPreviewUrl = `${environment.serverUrl}${updatedProfileWithAvatar.avatarUrl}`;
                }
                if (this.fileInputRef?.nativeElement) {
                    this.fileInputRef.nativeElement.value = '';
                }
