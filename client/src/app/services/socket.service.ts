...
export class SocketService implements ISocketService {
    ...
    private setUpListeners(): void {
        ...
        this.socket.on(GameRoomEvents.UpdateScore, (winnerId) => {
            ...
            if (
                (mainPlayer.id === winnerId || (winner?.isVirtual && this.getId() === this.gameManagerService.room.organisatorId)) &&
                result >= MAX_WINS
            ) {
        ...
        this.socket.on(GameRoomEvents.OrganizatorChanged, (data) => {
            this.gameManagerService.room.organisatorId = data.newOrganisatorId;
        });
    }
    ...
    private handlePlayerTurn(nextPlayer: Player, currentPlayer: Player, room: Room): void {
        ...
        const isOrganizerAndVirtualTurn = this.socket.id === room.organisatorId && nextPlayer.isVirtual;
