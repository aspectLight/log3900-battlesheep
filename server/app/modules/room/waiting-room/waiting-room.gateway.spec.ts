...
import { GameRoomEvents } from '@app/modules/room/game-room/game-room.gateway.events';
import { GameRoomService } from '@app/modules/shared-room/services/game-room.service';
import { WaitingRoomGateway } from '@app/modules/room/waiting-room/waiting-room.gateway';
...
describe('WaitingRoomGateway', () => {
    ...
    it('should create a waiting room and emit the created event', () => {
        ...
        gateway.handleCreateRoom({ roomId, gameId, host }, socket);

        expect(waitingRoomService.createRoom.calledWith(roomId, gameId, host, socketId));
    ...
    it('should return an error if the room is locked', () => {
        ...
        const gameId = 'game456';
        const socketId = 'socket789';
        const mockRoom = { id: roomId, gameId, hostId: host.id, roomId, players: [], isLocked: true, messages: [], journalEntries: [] };
    ...
    it('should allow a user to reserve an avatar', () => {
        ...
        gateway.handleReserveAvatar({ roomId, chosenAvatar, playerId: host.id }, socket);

        expect(reserveCharacterSpy).toHaveBeenCalledWith(roomId, host.id, chosenAvatar);
        expect(waitingRoomService.findRoomById.calledWith(roomId)).toBeTruthy();
        expect(server.to.calledWith(roomId)).toBeTruthy();
    });
    ...
    it('should allow a user to start the game', async () => {
        ...
        await gateway.handleStartGame(roomId);
    ...
    it('should not allow a user to start the game if the room is not locked', () => {
        ...
        gateway.handleStartGame(roomId);
    ...
    it('should handle player disconnection and notify rooms', () => {
        const socketId = 'socket789';
        const host1 = { id: 'org1', name: 'Alice' };
        const host2 = { id: 'org2', name: 'Bob' };
        const player = { id: 'pla1', name: 'Alice -2' };
        const roomId1 = 'room1';
        const roomId2 = 'room2';
        const gameId1 = 'game456';
        const gameId2 = 'game456';
        const mockRoom1 = { id: roomId1, gameId1, hostId: host1.id, roomId: roomId1, players: [host1, player], isLocked: false };
        const mockRoom2 = { id: roomId2, gameId2, hostId: host2.id, roomId: roomId2, players: [host2, player], isLocked: false };
